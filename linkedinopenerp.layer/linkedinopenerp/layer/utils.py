...
class OpenErpAPI(object):
    ...
    def search_employees(self):
        ...
        employees = self.conn.read('hr.employee', employee_ids, search_employee_fields)
        return employees
    
    def search_contacts(self):
        qargs = [('linkedin', '=', True), ('name', '!=', ''), ('first_name', '!=', '')]
        contact_ids = self.conn.search('res.partner.contact', qargs)
        contacts = self.conn.read('res.partner.contact', contact_ids, search_contact_fields)
        return contacts
    ...
    def update_positions(self, employee, linkedin_positions=[], oe_object=None):
        for pos in linkedin_positions:
            ...
            if pos_results:
                ...
                self.update_position(employee['id'], pos_id['id'], pos, oe_object)
            else:
    ...
    def get_linkedin_values(self, li_empl, li_extrainfo):
        values = {
            ...
            'linkedin_specialties': self.erpize_value(li_extrainfo.specialties),
            ...
            'linkedin_picture_url': self.erpize_value(li_extrainfo.picture_url),
            'linkedin_skills': self.erpize_value(li_extrainfo.skills),
            'linkedin_public_url': self.erpize_value(li_extrainfo.public_url),
            'linkedin_location_country': self.erpize_value(li_empl.location_country),
